                    ...
                    dropdownMenu = boxDropdown(
                      ...
                      , icon=icon("download"))  )),
                       ...
                       box(title = "Initial Parameters", solidHeader = T, status = "primary", width = 12, collapsible = T,
                           ...
                           actionBttn(ns("QC_submit_sidebarPanel"),label="Apply",style = "jelly",color = "success",icon = icon("sliders")),
                           tags$br(),
                           tags$br(),

                           downloadBttn(ns('normalized_data'), '.rds normalized data')
                          ))#
...
normalizationServer <- function(id,filtered_data) {
  moduleServer(
    ...
    function(input, output, session) {
      ...
      download_plot_Server("normalization_plot", input_data = plot , name = paste0('normalization_by_', input$INPUT_Nrmlz_Method))
